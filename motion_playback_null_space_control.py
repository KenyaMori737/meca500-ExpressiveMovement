...
def motion_playback_null( x_dot ):
    ...
    for i in range(len(x_dot)-1):
        ...
        # 第一引数 omoto yに動く
        # 第二引数 omoto xに動く
        twist_ref_main = np.array([x_dot[i][0],x_dot[i][1],x_dot[i][3],x_dot[i][5]])
        twist_ref_sub = np.array([x_dot[i][8],x_dot[i][10]])
        
        q = np.dot( np.array(robot.GetJoints()), np.pi/180.0)
        J = calc_eff_jacobian2(q)
        J2 = J[ [0,1,3,5], :]
        J2psu = np.dot( J2.transpose(), np.linalg.inv( np.dot(J2, J2.transpose()) + np.dot(np.identity(4), 0.00001) ) )
        J2_Ipsu = ( np.identity(6)-np.dot(J2psu,J2) );

        J3 = J[ [2,4], :]
        J3psu = np.dot( J3.transpose(), np.linalg.inv( np.dot(J3, J3.transpose()) + np.dot(np.identity(2), 0.00001) ) )
        J3_bar = np.dot(J3, J2_Ipsu)
        J3_bar_psu = np.dot( J3_bar.transpose(), np.linalg.inv( np.dot(J3_bar, J3_bar.transpose()) + np.dot(np.identity(2), 0.00001) ) )

        #tmp_dq = np.dot(J2psu, twist_ref_main)
        tmp_sub_task = twist_ref_sub - np.dot(J3, np.dot( J2psu, twist_ref_main) )
        tmp_dq = np.dot(J2psu, twist_ref_main) + np.dot(J3_bar_psu, tmp_sub_task )
        
        dq_ref = np.dot( tmp_dq, 180.0/np.pi)
        #invJ = np.linalg.inv(J)
        #dq_ref = np.dot( np.dot(invJ, twist_ref_main ), 180.0/np.pi)
...
def main():
    ...
    with open(r'./motions/motion_offline_ik_3_hw.csv') as f:
        reader = csv.reader(f)
        x_dot = [[float(v) for v in row] for row in reader]
    log_ang = motion_playback_null( x_dot )
